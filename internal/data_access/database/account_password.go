...
func (a accountPasswordDataAccessor) GetAccountPassword(ctx context.Context, ofAccountID uint64) (AccountPassword, error) {
	...
	found, err := a.database.From(TabNameAccountPasswords).Where(goqu.Ex{ColNameAccountPasswordsOfAccountID: ofAccountID}).ScanStructContext(ctx, &accountPassword)
	if err != nil {
	...
	if !found {
		logger.Warn("account password not found by id")
...
func (a accountPasswordDataAccessor) UpdateAccountPassword(ctx context.Context, accountPassword AccountPassword) error {
	...
	_, err := a.database.Update(TabNameAccountPasswords).Set(goqu.Record{
		ColNameAccountPasswordsPasswordHash: accountPassword.PasswordHash,
	}).Where(goqu.Ex{ColNameAccountPasswordsOfAccountID: accountPassword.OfAccountID}).Executor().ExecContext(ctx)
	if err != nil {
